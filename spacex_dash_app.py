...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        data_df = filtered_df[filtered_df['class']==1].groupby(by='Launch Site').sum().reset_index()
        fig = px.pie(data_df, values='class', 
        names='Launch Site', 
        title='Successful Launches by Site')
        return fig
    else:
        data_df = filtered_df[filtered_df['Launch Site']==entered_site].groupby(by='class').count().reset_index()
        fig = px.pie(data_df, values='Launch Site', 
        names='class', 
...
def get_scatter_chart(entered_site,payload_range):
    ...
    if entered_site == 'ALL':
        fig = px.scatter(filtered_df, 
            ...
            title='Launch Success by payload mass and booster version')
        return fig
    else:
